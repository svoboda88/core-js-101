...
function timeSpanToString(startDate, endDate) {
  ...
  const zeroFunc = (time) => (time < 10 ? `0${time}` : time);
  const doubleZeroFunc = (time) => (time < 10 ? `00${time}` : time);
  return `${zeroFunc(hours)}:${zeroFunc(minutes)}:${zeroFunc(seconds)}.${doubleZeroFunc(milliseconds)}`;
}
